...
public class HttpClientUtil {
     ...
     *  该方法会自动关闭连接,释放资源
     *  requestURL    请求地址(含参数)
     *  decodeCharset 解码字符集,解析响应数据时用之,其为null时默认采用UTF-8解码
     * @return 远程主机响应正文
     */
    ...
    public static String sendGetRequest(String reqURL,Map<String,String> headerMap){
        ...
        responseContent = getString4Response(httpClient, httpGet);
        return responseContent;
    }

    private static String getString4Response(HttpClient httpClient, HttpGet httpGet) {
     ...
     *  isEncoder 用于指明请求数据是否需要UTF-8编码,true为需要
     */
     ...
     *  reqURL        请求地址
     *  sendData      请求参数,若有多个参数则应拼接成param11=value11&22=value22&33=value33的形式后,传入该参数中
     *  isEncoder     请求数据是否需要encodeCharset编码,true为需要
     *  encodeCharset 编码字符集,编码请求数据时用之,其为null时默认采用UTF-8解码
     *  decodeCharset 解码字符集,解析响应数据时用之,其为null时默认采用UTF-8解码
     * @return 远程主机响应正文
     */
     ...
     *  reqURL 请求地址
     *  params 发送到远程主机的正文数据,其数据类型为<code>java.util.Map<String, String></code>
